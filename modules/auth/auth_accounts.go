...
import (
	...
	//"github.com/maoaixiao1314/treasureapp/crypto/codec"
...
func GetGenesisAccounts(appState map[string]json.RawMessage, cdc codec.Codec) ([]types.Account, error) {
	...
	fmt.Println("测试断点1")
	fmt.Printf("appState:%+v\n", string(appState["auth"]))
	fmt.Printf("authttypes.ModuleName:%+v\n", authttypes.ModuleName)
	fmt.Printf("authState :%+v\n", authState)
	if err := cdc.UnmarshalJSON(appState[authttypes.ModuleName], &authState); err != nil {
		return nil, err
	}
	fmt.Println("测试断点121212")
	// Store the accounts
	fmt.Printf("authState.Accounts:%+v\n", authState.Accounts)
